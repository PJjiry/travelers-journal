...
const PlaceForm: React.FC<{place?:Place, title:string}> = ({place, title}) => {
    ...
    return (
        <form className={classes.form} onSubmit={handleSubmit}>
            ...
            <Input title="Name of the place" name="title" value={place?.title || placeForm.title} onInputChange={handleChange}
            ...
            imageUrl={place?.imageUrl || placeForm.imageUrl} onRemoveImage={handleRemoveImage} onImageChange={handleImageChange} onImageDrop={handleImageDrop}/>
            <Select title="Type of place" name="type" value={place?.type || placeForm.type}
            ...
            <Input title="Date of visit" name="date" value={place?.date || placeForm.date} onInputChange={handleChange}
                   tooltip="Pick a date you have visited the place or are planning to visit." type="date" />
            <Input title="Country" name="country" value={place?.country || placeForm.country} onInputChange={handleChange}
                     tooltip="Type a country where is the place located." />
            <Select title="Continent" name="continent" value={place?.continent || placeForm.continent}
            ...
            {(place &&  isPastDate(place?.date)) && <Input title="Budget for the whole trip in $USD (optional)" name="budget" value={place?.budget} onInputChange={handleChange}
                                                                       tooltip="Enter a budget spent for the whole trip." min={0} type="number"/>}
            <Input title="Description of the place" name="description" value={place?.description || placeForm.description} onInputChange={handleChange}
                     tooltip="Add some basic description about the place." isTextarea={true}/>
            <Input title="Special requirements (optional)" name="specialRequirements" value={place?.specialRequirements || placeForm.specialRequirements} onInputChange={handleChange}
                        tooltip="Mention special requirements that were needed to visit the place like visa or booking in advance." isTextarea={true}/>
            {(placeForm.type === "City" || place?.type==="City") &&
                <SightForm sights={place?.sights || placeForm.sights} sightName={sight.sightName} sightDescription={sight.sightDescription} onAddSight={() => {
                    handleAddSight(sight.sightName, sight.sightDescription);
                }} onRemoveSight={handleRemoveSight} onSightChange={handleSightChange}/>}
            <LocationForm location={place?.location || placeForm.location} onLocationChange={handleLocationChange} />
